...
class StatTree(object):
    ...
    def plot(self,path, query_granularity=0):
        print("StatTree Plotting...")
        self.update_stat_nodes_granularity()
        graph = XDigraph("Computational graph", filename=path, engine="dot")
        # graph.attr(size="6,6")
        # graph.node_attr.update(color='lightblue2', style="filled")
        # graph.graph_attr.update(rankdir="BT")

        graph.add_node_subgraph_to_plot_graph(self.root_node)   #recursive function
        # dot.format = format
...
class StatNode(object):
    ...
    def add_context(self,ctx,ctx_list):
        self.context_list.append(ctx + "_" )       #+ str(time.time())
        if(len(ctx_list)>0):
            self.context_list = ctx_list+self.context_list
