...
class DistillWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.distillation_token = nn.Parameter(torch.randn(1, 1, dim)).cuda()
        # self.distillation_token = self.distillation_token.cuda()

        self.distill_mlp = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, num_classes)
        )
        self.distill_mlp.cuda()
    ...
    def forward(self, img, labels, temperature = None, alpha = None, **kwargs):
        ...
        distill_loss = F.kl_div(
            ...
            F.softmax(teacher_logits[:,:self.num_classes] / T, dim = -1).detach(),
        reduction = 'batchmean')
